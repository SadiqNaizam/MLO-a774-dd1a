...
import {
  ...
  ChevronDown,
  PlusCircle,
  Settings2, // Used as placeholder for ... (More options) next to News Feed
  Newspaper, // For News Feed
  UserCircle2, // For Olenna Mason (profile)
  HelpCircle, // For Fundraisers (placeholder)
  Gamepad2, // For FarmVille 2
  MoreHorizontal // For generic 'more' or options
} from 'lucide-react';
...
interface NavItem {
  ...
  isActive?: boolean;
  action?: () => void;
  // items?: NavItem[]; // For sub-items like "See More..." if complex nesting needed
}
...
const SidebarNav: React.FC<SidebarNavProps> = ({ className }) => {
  ...
  const mainNavItems: NavItem[] = [
    // Profile is handled separately above these items
  ...
  const shortcuts: NavItem[] = [
    ...
    // Add more shortcuts here if any
  ];
  ...
  const createItems: { id: string; label: string; action: () => void; }[] = [
      ...
      { id: 'fundraiser', label: 'FundraiserCreate', action: () => console.log('Create Fundraiser') }, // Renamed to avoid conflict
  ];
  ...
  return (
    <aside className={cn("w-[280px] bg-sidebar text-sidebar-foreground fixed top-0 left-0 h-screen flex flex-col pt-[60px]", className)}>
      <ScrollArea className="flex-grow">
        <nav className="p-2 space-y-1">
          ...
          {mainNavItems.map((item) => (
              <Button
                key={item.id}
                variant={'ghost'} // Active state handled by conditional classNames
                className={cn(
              ...
              >
                <item.icon className={cn("h-5 w-5 mr-3", item.isActive ? "text-sidebar-primary" : "text-sidebar-primary")} /> {/* Icon color often primary for emphasis */}
                {item.label}
          ...
          <div className="px-1 py-2">
            <h2 className="mb-1 px-2 text-sm font-semibold text-muted-foreground">Shortcuts</h2>
            {shortcuts.length > 0 ? shortcuts.map((item) => (
              <Button
                ...
                onClick={item.action || (() => { if(item.href) console.log(`Navigating to ${item.href}`)})}
              >
                <item.icon className="h-5 w-5 mr-3 text-sidebar-foreground" /> {/* Shortcut icons might be plain foreground */}
                {item.label}
              </Button>
          ...
          {/* Explore */} 
          <div className="px-1 py-2">
            <h2 className="mb-1 px-2 text-sm font-semibold text-muted-foreground">Explore</h2>
          ...
          <div className="px-3 py-2">
            <h2 className="mb-2 text-sm font-semibold text-muted-foreground">Create</h2>
      ...
      <div className="p-4 border-t border-sidebar-border text-xs text-muted-foreground text-center">
        ...
        <MoreHorizontal className="inline h-3 w-3 mx-1" /> Cookies ·{' '}
