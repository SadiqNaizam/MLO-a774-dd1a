...
const Header: React.FC<HeaderProps> = ({ className }) => {
  ...
  const toggleTheme = React.useCallback(() => {
    setTheme(prev => {
      ...
      // In a real app: document.documentElement.classList.toggle('dark', newTheme === 'dark');
  ...
  return (
    <header
      className={cn(
        "fixed top-0 left-0 right-0 z-10 flex items-center justify-between px-6 h-[60px] bg-card border-b border-border shadow-sm",
        className
      )}
    >
      {/* Left Section: Logo and Search */} 
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" className="text-primary rounded-full hover:bg-accent focus-visible:ring-ring p-0">
      ...
      {/* Middle Section: Navigation Tabs (Simplified) */} 
      <nav className="hidden lg:flex items-center h-full">
        <Button variant="ghost" className="flex-col justify-center h-full px-10 text-primary hover:bg-accent rounded-none border-b-4 border-primary focus-visible:ring-0 focus-visible:ring-offset-0">
          <Home className="h-7 w-7" />
          <span className="sr-only">Home</span>
        </Button>
        <Button variant="ghost" className="flex-col justify-center h-full px-10 text-muted-foreground hover:text-primary hover:bg-accent rounded-none focus-visible:ring-0 focus-visible:ring-offset-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="default" size="icon" className="relative rounded-full bg-secondary hover:bg-accent text-foreground focus-visible:ring-ring w-10 h-10">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="default" size="icon" className="rounded-full bg-secondary hover:bg-accent text-foreground focus-visible:ring-ring w-10 h-10">
          ...
          <DropdownMenuContent align="end" className="w-80 mt-2">
            <DropdownMenuLabel className="p-0">
                <div className="flex items-center gap-3 p-3 hover:bg-accent rounded-t-md cursor-pointer">
                    ...
                    <div>
                        <p className="font-semibold text-base text-card-foreground">{user.name}</p>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="text-red-600 focus:bg-red-50 focus:text-red-700 dark:text-red-500 dark:focus:bg-red-700/20 dark:focus:text-red-400 cursor-pointer">
